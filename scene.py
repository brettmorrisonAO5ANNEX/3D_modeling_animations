...
class CubicBezier(ThreeDScene):
    def construct(self):
        ...
        #3. define control points
        bezier_scene.create_control_point(np.array((-2, 0, 0)), id=0, u=0, v=0)
        bezier_scene.create_control_point(np.array((-1.5, 2, 0)), id=1, u=0, v=1)
        bezier_scene.create_control_point(np.array((1.5, 2, 0)), id=2, u=0, v=2)
        bezier_scene.create_control_point(np.array((2, 0, 0)), id=3, u=0, v=3)
        ...
        self.wait()
        self.play(Create(quadratic_bezier_1.path),
                  Create(quadratic_bezier_2.path),
                  Create(cubic_bezier.path))
        self.wait()
        """
